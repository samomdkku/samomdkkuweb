...
- ✅ **DEPLOYED = `2a59df4` (2026-09-22)** — v4.7.0 still, read back from the VM's
  own HEAD, not retyped. `<== exit 0 — ran to the end` + `DEPLOY_EXIT=0`; docs step RAN (web 10:07:01Z, docs 10:07:15Z). SERVED: the live `core-*` chunk carries the second-pass retry string; `/docs/SHOP-GALLERY` serves its own content (not the SPA fallback). The day's work — slip bug, pick-time sweep, full shop sweep (0202), second pass, gallery DESIGN — and how each was checked: `docs/state/claude-2026-09-22.md`. Previous: `95a31e0` (`docs/state/claude-2026-09-21.md`).
...
- **Migrations through 0203. 47 of 51 LIVE PROOFS GREEN on production (2026-09-22) — the 4 red are house/team/dept proof-subject drift: `docs/state/HANDOFF.md` §16b.** 0202 shop sweep: stock shown = stock sold, colour/slip/qty checked, a buyer's own-order edits limited to what the slip flows write (`shop0202-stock-rule` 22/22). 0203 product gallery — several pictures, cover derived by trigger (`shop0203-gallery` 16/16). 0199 shop per-size pricing (`shop0199-pricing` 20/20); 0200 the three copies of a person stay in sync + admin mismatch panel (`house0200-three-copies` 13/13, `house0194` back to 20/20), both dev AND production — what each one changed and which hole it closed is `docs/state-archive/2026-09-19-state-prune.md`; the proofs themselves are the authority. ⛔ Ask `npm run migrate:status`, never this line.
