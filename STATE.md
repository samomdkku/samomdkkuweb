...
- ✅ **DEPLOYED = `ccd127c` (2026-09-14)** — v4.7.0 still, read back from the VM's
  own HEAD, not retyped. `<== exit 0 — ran to the end` + `DEPLOY_EXIT=0`, **31 s**
  (baseline); docs step RAN, roots 13 s apart. SERVED: `ข้อมูลไม่ครบ` + `หายไปจากหลายรุ่นพร้อมกัน` each grep 1 in `admin-CngP2_JL.js`, control `ยังนำเข้าไม่ได้` 3; the 2 remaining `vssound` refs are BUG links, which stay. เชื่อมบัญชี Discord shipped in `08b48b0` — the
...
✅ **ข้อมูลไม่ครบ tab** (/admin/ ระบบบ้าน) — everything missing/mismatched/odd, grouped by who can fix it; badge counts ONLY admin work. One definition, shared with `npm run house:gaps`: `src/js/house/gaps.js`.
