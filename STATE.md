...
- ✅ **DEPLOYED = `11e2d2a` (2026-09-19)** — v4.7.0 still, read back from the VM's
  own HEAD, not retyped. `<== exit 0 — ran to the end` + `DEPLOY_EXIT=0`; docs step
  RAN. SERVED: `/docs/state/HANDOFF` carries `FOLLOWS ทีม SAMO BY ITSELF` (1); `samo-discord-sync` active after the restart.
...
- **Migrations through 0198. ALL 47 LIVE PROOFS GREEN** (2026-09-14; 0195–0198 added and the Discord proofs re-run 2026-09-19) — what each one changed and which hole it closed is `docs/state-archive/2026-09-19-state-prune.md`; the proofs themselves are the authority. ⛔ Ask `npm run migrate:status`, never this line.
