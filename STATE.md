...
- ✅ **DEPLOYED = `75ef966` (2026-09-13)** — v4.7.0 still, read back from the VM's
  own HEAD, not retyped. `<== exit 0` + `DEPLOY_EXIT=0`, 30 s. Docs step RAN:
  `THE NGINX DRIFT IS CLOSED` greps 1 on the served `/docs/state/HANDOFF`,
  beside a shipped control; `npm run check:routes` green against prod. เชื่อมบัญชี Discord shipped in `08b48b0` — the
