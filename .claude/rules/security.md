...
| Discord **bot** token (`DISCORD_TOKEN=…`) | ❌ NEVER | The role-sync bot's login. **NOT a webhook** — a webhook posts to one channel; this token *is* the bot account and can read the member list, and add or remove any role below it. ⛔ **Leaked into a chat transcript THREE TIMES — 2026-08-28, 2026-09-11 and 2026-09-12 — so ANY value that has ever existed must be treated as public and RESET before use.** The third was an INSPECTION script built to avoid exactly this: its masking keyed on `=` and the file's defect was a bare value with no key, so the substitution matched nothing and passed the token through (`docs/mistakes/tooling-proofs.md`). **Never read this file with a tool that CAN print a line** — emit lengths and booleans only.** Lives only in the VM's systemd environment file (`/etc/samo-discord-bot.env`, root-owned, `0600`); never in git, never in `.env.local` on a laptop, never pasted into chat. **Why not `.env.local`, when that file already holds worse?** (owner asked, 2026-09-12 — a fair question: `SUPABASE_DB_URL` there bypasses every RLS policy over real student records, and `SAMO_VM_SUDO_PASSWORD` is root on the production host.) **The rule is about SHARING, not about the file, and it was stated too broadly.** For the OWNER, whose laptop already holds both of those, a local copy is not the sloppier place — it is the same place, and it is fine. What must never happen, for anyone: in git, in a chat message, in a `docs/` page, or handed to a contributor. Both of this token's leaks were a copy in TRANSIT; none was the VM's file. A contributor needs no token at all — see the row below | Reset at discord.com/developers → the app → Bot → Reset Token |
| Discord **dev** bot token | ⛔ **PROPOSED AND DECLINED 2026-09-12 — do not re-propose** | A second Discord application, invited without `Manage Roles`, was proposed so local work could not write. The owner declined it: *"create second discord application is tiring, i want to can test this bot that work on prod not new one."* Accepted — the upkeep is real (two apps, two invites, two token resets, a test guild whose role tree drifts from the real one) and the safety it bought is available another way. **What replaces it:** the half of the sync that decides which roles a person SHOULD hold is `public.discord_role_targets()` in Postgres, so it is developed and proved against samo-dev with no Discord credential at all; the half that READS the live guild runs on the VM. Writes are gated by `--apply` defaulting off AND the blast-radius cap, and the reconcile module is asserted to contain no Discord write call |
| Discord **webhook** URLs (`NOTIFY_DISCORD_*_WEBHOOK`, `DISCORD_SHOP_WEBHOOK` — new shop orders, VM only, and `DISCORD_SYNC_LOG_WEBHOOK` — the role-bot channel's change log, VM only) | ❌ NEVER | `.env.local` + the VM's `/etc/samo-notify.env`. These are what every SAMO notification actually uses. **Independent of the bot token** — resetting the bot changes nothing here |
