...
| Discord **webhook** URLs (`NOTIFY_DISCORD_*_WEBHOOK`, and `DISCORD_SYNC_LOG_WEBHOOK` — the role-bot channel's change log, VM only) | ❌ NEVER | `.env.local` + the VM's `/etc/samo-notify.env`. These are what every SAMO notification actually uses. **Independent of the bot token** — resetting the bot changes nothing here |
