...
| Discord **bot** token (`DISCORD_TOKEN=…`) | ❌ NEVER | The role-sync bot's login. **NOT a webhook** — a webhook posts to one channel; this token *is* the bot account and can read the member list, and add or remove any role below it. ⛔ **Leaked into a chat transcript TWICE — 2026-08-28 and 2026-09-11 — so any value predating 2026-09-12 must be treated as public and RESET before use.** Lives only in the VM's systemd environment file (`/etc/samo-discord-bot.env`, root-owned, `0600`); never in git, never in `.env.local` on a laptop, never pasted into chat. **Why not `.env.local`, when that file already holds worse?** (owner asked, 2026-09-12 — a fair question: `SUPABASE_DB_URL` there bypasses every RLS policy over real student records.) Because the answer is not a safer FILE, it is a second CREDENTIAL — `DISCORD_DEV_TOKEN` below. A value that exists in two places leaks from the sloppier one, and both of this token's leaks were a copy in transit, not the VM's file. Reset at discord.com/developers → the app → Bot → Reset Token |
| Discord **dev** bot token (`DISCORD_DEV_TOKEN=…`) | ❌ not in git, ✅ **shareable with the team** | `.env.local` only — a SECOND Discord application for local work on the role sync, added 2026-09-12. Same tier, and for the same reason, as `GOOGLE_DEV_*`: it is dev-only by CONSTRUCTION, not by intent. ⛔ **Invite it with NO `Manage Roles`.** The sync exists to remove roles from people, so a dev bot that can write is a production credential wearing a dev label; a report-only run needs only the SERVER MEMBERS intent, which is a read. A permission never granted cannot be lost to a bug, a bad merge or a stray `--apply`; a rule in the code saying "report only" can be lost to all three. ⛔ **Production has its own, separate application and this must never become it** — dev credentials are deliberately shared with the whole team |
