...
| Discord **webhook** URLs (`NOTIFY_DISCORD_*_WEBHOOK`) | ❌ NEVER | `.env.local` + the VM's `/etc/samo-notify.env`. These are what every SAMO notification actually uses. **Independent of the bot token** — resetting the bot changes nothing here |
