...
| Discord **bot** token (`DISCORD_TOKEN=…`) | ❌ NEVER | The role-sync bot's login (app `1492541609445949465`). **NOT a webhook** — a webhook posts to one channel; this token *is* the bot account and can read the member list, and add or remove any role below it. ⛔ **Leaked into a chat transcript TWICE — 2026-08-28 and 2026-09-11 — so the value that exists today must be treated as public and RESET before use.** Lives only in the VM's systemd environment file (`/etc/samo-discord-bot.env`, root-owned, `0600`); never in git, never in `.env.local` on a laptop, never pasted into chat. Reset at discord.com/developers → the app → Bot → Reset Token |
